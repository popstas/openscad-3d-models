...
- **Одноразовый рендер модели**: `npm run render -- models/<папка>` — принудительно собирает STL + 4 превью, пишет артефакты валидации (см. ниже) и обновляет индекс `models/README.md`. Основная команда для проверки модели после правок (параметры камеры берутся из `PNG_VIEWS` в `src/render-core.ts` — не дублировать их вручную)
...
### Артефакты валидации рендера
При каждом рендере рядом с моделью автоматически создаются (правило: сначала проверяй числа, потом смотри картинки). В git не коммитятся (как и STL) — если файлы отсутствуют или устарели, запусти `npm run render -- models/<папка>`:
- **`<модель>.geometry.json`** — машиночитаемый отчёт: `dims`/`bbox` (габариты), `minZ` (должен быть ~0 — иначе деталь висит над столом), `parts` (связные компоненты = отдельные детали, у каждой bbox/offset/объём; лишняя микро-деталь = мусор булевых операций), `facets`, `cgal` (`manifold`, `warnings` — «unknown variable» означает молча выпавшую геометрию), `expected` (результат сверки с ожиданиями)
- **`<модель>.cut-x/y/z.png`** — поперечные срезы из STL по серединам осей: толщина стенок, карманы, внутренние полости. Сетка 10 мм, тёмная линия — координата 0
- **Ожидаемая геометрия** — объявите в SCAD литералы (не выражения!), и каждый рендер сверит их с фактом (`EXPECT OK` / `EXPECT MISMATCH` в выводе):
```scad
expected_dims = [116, 106.4, 30]; // внешние габариты всей раскладки [X, Y, Z], мм
expected_parts = 2;               // число отдельных деталей на столе
expected_tol = 0.5;               // необязательный допуск, мм (по умолчанию 0.5)
```
- Для инвариантов внутри модели используйте `assert()` — рендер упадёт с внятной ошибкой вместо молча неверной геометрии
...
В начале каждого файла модели подключайте общий файл модулей из корня каталога моделей:
```scad
use <../modules.scad>;
```
