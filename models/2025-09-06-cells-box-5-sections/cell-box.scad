...
sections_x = is_small ? [10, 20, 30] : [15, 15, 15, 15, 15];
// глубина по Y
inner_y    = is_small ? 20 : 150;
// внутренняя глубина во всех отсеках
inner_h    = is_small ? 10 : 31 + 2;
    
// Зона для этикеток спереди внутри коробки: добавляется к inner_y
labels_w    = 10;          // ширина зоны под этикетки по Y (мм)
labels_text = ["AG13", "1632", "2025", "2032", "2450"];
labels_h    = 0.6;
labels_fs   = 4; // размер шрифта по высоте зоны
...
cap_top_th        = 1.6;     // толщина верхней пластины крышки
cap_lip_h         = 16;     // высота юбки (захват за стенки)
...
outer_y = 2*wall_th + inner_y_e + labels_w; // дополнительная глубина под этикетки
...
section_y = wall_th + labels_w; // отступ секций назад, чтобы спереди была зона labels
...
// Надписи на верхней плоскости (рельефом вверх) над каждой секцией
module labels(){
    y_pos = wall_th + labels_w/2; // центр зоны под этикетки
    for (i = [0 : n_sections - 1]){
        has_text = (i < len(labels_text)) && (len(labels_text[i]) > 0);
        if (has_text){
            x_center = section_x_at(i) + sections_x_e[i]/2;
            translate([x_center, y_pos, outer_h + 0.02])
                linear_extrude(height=labels_h)
                    text(labels_text[i], size=labels_fs, halign="center", valign="center");
        }
    }
}
...
module all_parts(){
    ...
    if (print_labels) color("black") labels();
}
