...
phone_h = 131 - 0;         // высота телефона (идёт в глубину кармана) по Y
...
notch_depth_ratio = 0.25; // доля глубины для нижнего выреза (0..1)
...
module pocket_cut(){
    ...
    translate([wall_xy, 0, 0])
        rounded_rr_extrude(
            size=[pocket_w, pocket_d*notch_depth_ratio],
            r=notch_r_xy,
            h=notch_h,
            s=1,
            mink_r=pocket_mink_r
