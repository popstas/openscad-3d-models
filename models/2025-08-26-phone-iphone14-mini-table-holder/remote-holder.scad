...
// ----------------------------
is_small = false;
phone_w = is_small ? 42 : 42; // ширина телефона по X, с боковыми кнопками
phone_h_gap = 10;
phone_h = is_small ? 30 : 30; // высота телефона (идёт в глубину кармана) по Y
phone_th = is_small ? 16.5 : 16.5; // толщина телефона по Z
notch_camera_z = is_small ? phone_h - 0 : 32 - phone_h_gap;

// Зазоры кармана (внутренние)
pocket_clear_x = 0.2; // общий запас по X (ширина) — суммарно (0 для точного соответствия)
pocket_clear_y = 0.2; // запас по Y (толщина)
...
print_notch = false; // output notch difference
