...
sections_x = is_small ? [10, 20, 30] : [44];
// глубина по Y
inner_y    = is_small ? 20 : 130;
...
// Толщины
all_th       = 1.6;        // общая толщина
wall_th      = all_th;          // толщина наружных стенок
divider_th   = all_th;          // толщина перегородок
bottom_th    = all_th;          // толщина дна

// Параметры крышки (cap)
cap_top_th        = all_th;     // толщина верхней пластины крышки
cap_lip_h         = 8;     // высота юбки (захват за стенки)
cap_fit_clearance = 0.15;   // зазор между наружными стенками коробки и внутренней поверхностью юбки
cap_th            = all_th; // толщина стенки крышки; по умолчанию согласована с margin и зазором
...
module all_parts(){
    ...
    if (print_cap) translate([outer_x + x_gap, 0, 0]) upside_down(cap_h) cap();
}
